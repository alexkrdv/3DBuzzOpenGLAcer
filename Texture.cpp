...
Texture::Texture(string _filename, string _name){
...
bool Texture::loadTGA(string filename){
	...
	if(!file.read((char*)&TGAheader,sizeof(TGAheader))) return false;

	if(TGAheader.ImageType != 2) return false;
